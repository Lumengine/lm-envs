"""Task registry — the single catalog of Lumotion tasks, with the metadata the
...
Tasks live in `lumotion_envs.tasks.*`; importing one bootstraps the engine
(see `lumotion_envs._engine`).
"""
...
REGISTRY: dict[str, TaskSpec] = {
    "Cartpole": TaskSpec(
        id="Cartpole", module="lumotion_envs.tasks.cartpole_task", cls="CartpoleTask", config_cls=CartpoleConfig,
        domain="classic", desc="Balance a pole on a force-controlled cart (fixed base).",
        default_envs=512, max_epochs=200),

    "Ant": TaskSpec(
        id="Ant", module="lumotion_envs.tasks.ant_task", cls="AntTask", config_cls=AntConfig,
    ...
    "Anymal": TaskSpec(
        id="Anymal", module="lumotion_envs.tasks.anymal_task", cls="AnymalTask", config_cls=AnymalConfig,
    ...
    "Go2": TaskSpec(
        id="Go2", module="lumotion_envs.tasks.legged_velocity", cls="LeggedVelocityTask", config_cls=Go2Config,
    ...
    "Go1": TaskSpec(
        id="Go1", module="lumotion_envs.tasks.legged_velocity", cls="LeggedVelocityTask", config_cls=Go1Config,
        domain="locomotion", desc="Unitree Go1 (MJCF, Menagerie) - velocity-command walking.",
        default_envs=4096, max_epochs=1000,
        train_kwargs={"horizon_length": 24, "mini_epochs": 5}, ppo_attr="LEGGED_PPO_PARAMS"),

    "A1": TaskSpec(
        id="A1", module="lumotion_envs.tasks.legged_velocity", cls="LeggedVelocityTask", config_cls=A1Config,
        domain="locomotion", desc="Unitree A1 (MJCF, Menagerie) - velocity-command walking.",
        default_envs=4096, max_epochs=1000,
        train_kwargs={"horizon_length": 24, "mini_epochs": 5}, ppo_attr="LEGGED_PPO_PARAMS"),

    "H1": TaskSpec(
        id="H1", module="lumotion_envs.tasks.legged_velocity", cls="LeggedVelocityTask", config_cls=H1Config,
        domain="locomotion", desc="Unitree H1 humanoid (MJCF, Menagerie) - bipedal velocity walking.",
        default_envs=4096, max_epochs=1500,
        train_kwargs={"horizon_length": 24, "mini_epochs": 5}, ppo_attr="LEGGED_PPO_PARAMS"),

    "FrankaReach": TaskSpec(
        id="FrankaReach", module="lumotion_envs.tasks.franka_reach", cls="FrankaReachTask", config_cls=FrankaReachConfig,
    ...
    "FrankaLift": TaskSpec(
        id="FrankaLift", module="lumotion_envs.tasks.franka_lift", cls="FrankaLiftTask", config_cls=FrankaLiftConfig,
    ...
    "FrankaCabinet": TaskSpec(
        id="FrankaCabinet", module="lumotion_envs.tasks.franka_cabinet", cls="FrankaCabinetTask",
    ...
    "AllegroCube": TaskSpec(
        id="AllegroCube", module="lumotion_envs.tasks.allegro_cube", cls="AllegroCubeTask",
...
def load_task(spec: TaskSpec):
    ...
    LUMENGINE_ROOT dev checkout — see lumotion_envs._engine)."""
